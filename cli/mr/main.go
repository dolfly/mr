	...
	app.Commands = []*cli.Command{
		&cli.Command{
			...
			Action: func(c *cli.Context) error {
				...
				s, err := mr.NewServer(c.String("listen"), c.String("password"), c.StringSlice("portPassword"))
				if err != nil {
					return err
				}
				go func() {
		...
		&cli.Command{
			...
			Action: func(c *cli.Context) error {
				...
				s := mr.NewClient(c.String("server"), c.String("password"), c.Int64("serverPort"), c.String("serverDomain"), cs, c.Int64("tcpTimeout"), c.Int64("tcpDeadline"), c.Int64("udpDeadline"))
				go func() {
					sigs := make(chan os.Signal, 1)
					signal.Notify(sigs, syscall.SIGINT, syscall.SIGTERM)
					<-sigs
